...
class Spotify {
    ...
    static async getFromURL(url, guildId = null) {
        try {
            ...
            if (!type || !id) {
                ...
                const errorMsg = lang.Music.Spotify.InvalidUrl || 'Invalid Spotify URL';
            ...
            switch (type) {
                ...
                default:
                    const lang = await getLang(guildId);
                    const errorMsg = (lang.Music.Spotify.UnsupportedType || 'Unsupported Spotify type: {type}').replace('{type}', type);
                    throw new Error(errorMsg);
            }
        } catch (error) {
            return [];
    ...
    static async formatTrack(spotifyTrack, guildId = null) {
        try {
            const lang = await getLang(guildId);
            const unknownArtist = lang.Music.Spotify.UnknownArtist || 'Unknown Artist';
            const unknownTitle = lang.Music.Spotify.UnknownTitle || 'Unknown Title';
