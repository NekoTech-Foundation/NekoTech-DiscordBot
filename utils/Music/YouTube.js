...
class YouTube {
        ...
        try {
            ...
            for (const item of results.entries.slice(0, limit)) {
                try {
                    ...
                    const unknownTitle = lang.Music.YouTube.UnknownTitle || 'Unknown Title';
                    const unknownArtist = lang.Music.YouTube.UnknownArtist || 'Unknown Artist';
    ...
    static async getInfo(url, guildId = null) {
        try {
            ...
            if (!info) {
                const lang = await getLang(guildId);
                const errorMsg = lang.Music.YouTube.NoInfo || 'No info returned from youtube-dl';
    ...
    static async getStream(url, guildId = null, startSeconds = 0) {
        try {
            ...
            if (!url) {
                const lang = await getLang(guildId);
                const errorMsg = lang.Music.YouTube.UrlRequired || 'URL required';
                throw new Error(errorMsg);
            }
            ...
            if (!info || !info.url) {
                const lang = await getLang(guildId);
                const errorMsg = lang.Music.YouTube.NoStreamUrl || 'No stream URL found';
                throw new Error(errorMsg);
            }
    ...
    static async getPlaylist(url, guildId = null) {
        try {
            ...
            if (!info) {
                const errorMsg = lang.Music.YouTube.NoPlaylistInfo || 'No playlist info found';
                throw new Error(errorMsg);
            }

            if (!info.entries || info.entries.length === 0) {
                const errorMsg = lang.Music.YouTube.NoPlaylistEntries || 'No playlist entries found';
                throw new Error(errorMsg);
            }
            ...
            if (tracks.length === 0) {
                const errorMsg = lang.Music.YouTube.NoValidTracks || 'No valid tracks found in playlist';
                throw new Error(errorMsg);
            }

            const unknownPlaylist = lang.Music.YouTube.UnknownPlaylist || 'Unknown Playlist';
