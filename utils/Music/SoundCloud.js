...
class SoundCloud {
    ...
    static async getInfo(url, guildId = null) {
        try {
            ...
            if (!info) {
                ...
                const errorMsg = lang.Music.SoundCloud.NoInfo || 'No info returned from SoundCloud';
    ...
    static async getStream(url, guildId = null, startSeconds = 0) {
        try {
            ...
            if (!result) {
                const lang = await getLang(guildId);
                const errorMsg = lang.Music.SoundCloud.NoStreamUrl || 'No stream URL found';
                throw new Error(errorMsg);
            }
    ...
    static async getPlaylist(url, guildId = null) {
        try {
            ...
            if (!result || !result.entries) {
                const lang = await getLang(guildId);
                const errorMsg = lang.Music.SoundCloud.NoPlaylistTracks || 'No playlist tracks found';
                throw new Error(errorMsg);
            }
            ...
            const lang = await getLang(guildId);
            const unknownPlaylist = lang.Music.SoundCloud.UnknownPlaylist || 'Unknown Playlist';
    ...
    static async formatTrack(soundcloudTrack, guildId = null) {
        try {
            const lang = await getLang(guildId);
            const unknownTitle = lang.Music.SoundCloud.UnknownTitle || 'Unknown Title';
            const unknownArtist = lang.Music.SoundCloud.UnknownArtist || 'Unknown Artist';
