...
class MusicPlayer {
    ...
    async addTrack(query, requestedBy, platform = 'auto') {
        try {
            ...
            const lang = await getLang(this.guild.id);
            if (!tracks || tracks.length === 0) {
                const errorMsg = lang.Music.Errors.NoResults || 'No results found.';
            ...
            const lang = await getLang(this.guild.id);
            const errorMsg = lang.Music.Errors.AddTrackError || 'Error adding track.';
            return { success: false, message: errorMsg };
        }
    }
        ...
        try {
            ...
            if (!this.currentTrack) {
                if (this.queue.length === 0) {
                    const errorMsg = lang.Music.Errors.EmptyQueue || 'Queue is empty.';
                    return { success: false, message: errorMsg };
                }
            ...
            if (!this.connection) {
                ...
                if (!connected) {
                if (!connected) {
                    const errorMsg = lang.Music.Errors.ConnectError || 'Could not connect to voice channel.';
                    return { success: false, message: errorMsg };
                }
                }
            }
                ...
                switch (this.currentTrack.platform) {
                    ...
                    case 'spotify':
                        ...
                        if (ytTrack && ytTrack.url) {
                            ...
                            const errorMsg = (lang.Music.Errors.SpotifyNoResult || "Could not find track '{title}' on YouTube.").replace('{title}', this.currentTrack.title);
                    ...
                    default:
                        const errorMsg = (lang.Music.Errors.UnsupportedPlatform || "Platform '{platform}' is not fully supported.").replace('{platform}', this.currentTrack.platform);
                        throw new Error(errorMsg);
                }
            }

            if (!streamInfo) {
                const errorMsg = lang.Music.Errors.StreamError || 'Could not retrieve audio stream.';
                throw new Error(errorMsg);
            }
            ...
            const lang = await getLang(this.guild.id);
            const errorMsg = lang.Music.Errors.PlayError || 'Error playing track.';
            return { success: false, message: errorMsg };
        }
    }
    ...
    async handleError(error) {
        ...
        if (this.queue.length > 0) {
            ...
            this.currentTrack = null;
            const lang = await getLang(this.guild.id);
            const errorMsg = lang.Music.Errors.PlaylistStopped || 'Playlist stopped due to an error.';
    ...
    async showQueueCompleted() {
        ...
        try {
            const lang = await getLang(this.guild.id);
            const completedTitle = lang.Music.Queue.Completed || 'Queue completed!';
            const completedDesc = lang.Music.Queue.CompletedDesc || 'Add more tracks to keep the music playing.';
    ...
    async restoreFromState(state) {
        ...
        if (this.textChannel && this.currentTrack) {
            try {
                const lang = await getLang(this.guild.id);
                const resumeMessage = lang.Music.Control.Resumed || 'Music resumed!';
