...
async function openQuestionModal(interaction) {
    try {
        ...
        if (!interaction.replied) {
            await interaction.reply({ content: lang.Suggestion.Error || 'Đã xảy ra lỗi.', flags: MessageFlags.Ephemeral });
        }
    }
}
...
command.addSubcommand(subcommand =>
    subcommand
        ...
        .addStringOption(option =>
            option.setName('id')
                .setDescription('ID của đề xuất để chấp nhận')
                .setRequired(true)
        )
        .addStringOption(option =>
    ...
    .addSubcommand(subcommand =>
        subcommand
            ...
            .addStringOption(option =>
                option.setName('id')
                    .setDescription('ID của đề xuất để từ chối')
                    .setRequired(true)
            )
            .addStringOption(option =>
                option.setName('reason')
    ...
    async execute(interaction, client) {
        try {
            if (!config.SuggestionSettings.Enabled) {
                await interaction.reply({ content: lang.Suggestion.SuggestionsDisabled || 'Tính năng đề xuất hiện đang bị tắt.', flags: MessageFlags.Ephemeral });
                return;
            }
            ...
            if (subcommand === 'create') {
                ...
                if (!hasAllowedRole) {
                    await interaction.reply({ content: lang.NoPermsMessage || 'Bạn không có quyền sử dụng lệnh này.', flags: MessageFlags.Ephemeral });
                    return;
                }

                const isBlacklisted = await SuggestionBlacklist.findOne({ userId: interaction.user.id });
                if (isBlacklisted) {
                    await interaction.reply({ content: lang.Suggestion.BlacklistMessage || 'Bạn đang trong danh sách đen và không thể tạo đề xuất.', flags: MessageFlags.Ephemeral });
                    return;
                }
                ...
                if (useQuestionModal) {
                    ...
                    try {
                        ...
                        await interaction.editReply({ content: lang.Suggestion.SuggestionCreated || 'Đề xuất đã được tạo thành công.' });
                    } catch (error) {
                        console.error("Lỗi khi tạo đề xuất:", error);
                        if (!interaction.replied) {
                            await interaction.reply({ content: lang.Suggestion.Error || 'Đã xảy ra lỗi.', flags: MessageFlags.Ephemeral });
                        } else {
                            await interaction.editReply({ content: lang.Suggestion.Error || 'Đã xảy ra lỗi.' });
                        }
                    }
                }
                ...
                const reason = interaction.options.getString('reason') || lang.Suggestion.Reason || 'Không có lý do được cung cấp';
                ...
                if (action === 'add') {
                    ...
                    await interaction.reply({ content: `${user} đã được thêm vào danh sách đen.`, flags: MessageFlags.Ephemeral });
                } else if (action === 'remove') {
                    await SuggestionBlacklist.deleteOne({ userId: user.id });
                    await interaction.reply({ content: `${user} đã được xóa khỏi danh sách đen.`, flags: MessageFlags.Ephemeral });
                }
            }
        } catch (error) {
